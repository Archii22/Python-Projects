...
# --------------------------
# Build natural language query (NLQ) → SQL or Pandas agent
# --------------------------
def nlq_to_code(nl_query: str, engine:str = "pandas") -> str:
    """
    Converts natural language query into Pandas or SQL code using OpenAI.
    
    engine: "pandas" or "sql"
    """
    prompt = f"""
...
with col2:
    # limit the number of rows rendered to avoid freezing the browser for very large files
    try:
...
if clean_drop_na_rows:
    try:
        ...
        st.error(f"Error dropping rows: {e}")
...
try:
    ...
    st.error(f"Failed to render cleaned preview: {e}")
...
try:
    ...
    st.error(f"Failed to compute dtypes: {e}")
...
try:
    ...
    st.error(f"Failed to compute outliers: {e}")
...
# Define the helper function at the top of your script, if not already defined
...
try:
    ...
    if anomaly_results:
        ...
        # Optional: plot each numeric column with anomalies
...
# Let user pick categorical columns (like City, Product)
categorical_cols = df.select_dtypes(include=["object", "category"]).columns.tolist()

if len(categorical_cols) == 0:
    st.info("No categorical columns found for segment analysis.")
else:
    ...
    if len(numeric_cols) == 0:
        ...
        try:
            ...
            fig = px.bar(
                ...
                x=segment_col,
                y=metric_col,
                title=f"Top {segment_col} contributing to {metric_col}",
                text_auto=True
            )
            st.plotly_chart(fig, use_container_width=True)
        except Exception as e:
...
def rank_insights(insights: list) -> pd.DataFrame:
    ...
    # Normalize impact & confidence between 0–1
...
if not ranked.empty:
    try:
        ...
        st.error(f"Failed to render ranked insights chart: {e}")


# Optional visualization
try:
    if not ranked.empty:
        st.bar_chart(ranked.set_index("insight")["score"])  # fixed index name
except Exception as e:
    st.error(f"Failed to render ranked insights chart: {e}")
...
# Dynamically create ranked insights
insights = []
...
#---------------------------------
# Generate narrative summary from ranked insights
if not ranked.empty:
...
insights_output = {
    "trends": trend_result,  # corrected from trends_summary
    "anomalies": anomaly_results,  # corrected from anomalies_summary
...
# --------------------------
# Connect to LLM (OpenAI GPT-4o mini or local Ollama)
# --------------------------
...
# --------------------------
# Narrative summaries of insights (bullets) 
# --------------------------
insights = [
...
summary_output = summarize_insights(insights)
st.subheader("📝 Narrative Insights Summary")
st.write(summary_output)

# --------------------------
# NLQ -> SQL or pandas agent
# --------------------------
...
if nlq_input:
    ...
    result = None  # Initialize variable to avoid NameError
...
if storage_option == "Pandas (in-memory)":
    ...
    try:
        ...
        st.error(f"Failed to render data head: {e}")
